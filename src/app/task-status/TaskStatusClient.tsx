    ...
    if (currentTaskId) {
      ...
      if (pendingUrl && pendingTaskId && !hasInitiatedAnalysis.current) {
        const startAnalysis = async () => {
          try {
            console.log(
              `Starting analysis for URL: ${pendingUrl}, Task ID: ${pendingTaskId}`
            );
            const response = await apiClient.post("/api/start-analysis", {
              url: pendingUrl,
              task_id: pendingTaskId,
            });
            console.log(
              "Analysis started successfully from TaskStatus:",
              response.data
            );
      ...
      const fetchUrlEntriesForTask = async () => {
        try {
          const response = await apiClient.get<UrlEntry[]>(
            `/api/urls?task_id=${currentTaskId}`
          );
          setUrlEntries(response.data);
        } catch (error) {
          console.error(
            `Error fetching URL entries for task ${currentTaskId}:`,
            error
          );
          setUrlEntries([]);
        }
    ...
    <div className="p-4 sm:p-8 lg:p-12 text-white">
      ...
      {urlEntries.length === 0 ? (
        <p className="text-gray-400 text-center">
          No URLs have been added yet for this task.
        </p>
      ) : (
